...
class Obsection extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('sectionTitle, sectionName, sectionTypeID, orderlist', 'required'),
			array('sectionTypeID, orderlist, parentID', 'numerical', 'integerOnly'=>true),
			array('sectionTitle, sectionName', 'length', 'max'=>50),
			array('sectionContent', 'length', 'max'=>60000),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('sectionTitle, sectionName, sectionTypeID, id, orderlist, parentID, sectionContent', 'safe', 'on'=>'search'),
		);
	}
